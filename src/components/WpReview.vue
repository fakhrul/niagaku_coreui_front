<template>
  <div>
    ...
    <CCard>
      ...
      <CCardBody>
        <h6>Required Exit Stair Width</h6>
        <div>
          <CRow>
            <CCol>
              <CForm>
...
export default {
    ...
    obj(newVal, oldVal) {
      ...
      if (self.obj.buildingCodeId) {
        self.minimumNumberOfExitApi
          .getByBuildingCode(self.obj.buildingCodeId)
          .then((response) => {
  ...
  computed: {
    ...
    buildingCode() {
      if (this.obj.buildingCode == null) return "";
      if (this.obj.buildingCode.name == null) return "";
      return this.obj.buildingCode.name;
    },
    mainClassification() {
      if (this.obj.olfMainCategory == null) return "";
      if (this.obj.olfMainCategory.name == null) return "";
      return this.obj.olfMainCategory.name;
    },
    ...
    totalStairExitWidthRequired() {
      ...
      var minimumEgressStairWidthInM =
        this.obj.paramEgress.minimumEgressStairWidthInMM / 1000;
  ...
  methods: {
    ...
    calcOccupantNumber(item) {
      try {
        ...
        var occupantNumber = area / item.occupantLoadFactor.olfValue;
    ...
    getTotalDoorWidthRequired(item) {
      console.log("getTotalDoorWidthRequired", item);
      if (item.occupantLoadFactor == null) return 0;
      // console.log('1',item);
    ...
    getParamEgressExitDoorWidthPerPersonInMmByClassId(classificationId) {
      try {
        for (
          ...
          if (
            this.obj.paramEgress.paramExitDoorWidthItems[i].olfMainCategoryId ==
            classificationId
          )
            return this.obj.paramEgress.paramExitDoorWidthItems[i]
              .widthPerPersonInMm;
    ...
    getMneCount(item) {
      ...
      if (self.obj.minimumNumberOfExit != null) {
        for (
          let i = 0;
      ...
      if (self.minimumNumberOfExit != null) {
        for (
          let i = 0;
          i < self.minimumNumberOfExit.minimumNumberOfExitItems.length;
          i++
        ) {
          if (
            ocl >=
              self.minimumNumberOfExit.minimumNumberOfExitItems[i]
                .occupantLoadFactorBegin &&
            ocl <=
              self.minimumNumberOfExit.minimumNumberOfExitItems[i]
                .occupantLoadFactorEnd
            ...
            console.log(
              "getMneCount YEA YEA",
              self.minimumNumberOfExit.minimumNumberOfExitItems[i].count
            );
            mne = self.minimumNumberOfExit.minimumNumberOfExitItems[i].count;
            break;
          }
        }
      }
