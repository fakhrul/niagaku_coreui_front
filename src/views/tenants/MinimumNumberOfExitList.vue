    ...
    <div>
      <CRow>
        <CCol sm="12">
          <CCard>
            <CCardHeader>
              <strong> MinimumNumberOfExit </strong> List
            </CCardHeader>
            <CCardBody>
              <CDataTable
                :items="computedItems"
...
const fields = [
  // { key: "buildingCodeName", _style: "min-width:200px;" },
  { key: "buildingCodeName", _style: "min-width:200px;" },
  { key: "totalParam", _style: "min-width:200px;" },
  {
    key: "show_details",
    label: "",
    _style: "width:1%",
...
export default {
    ...
    computedItems() {
      return this.items.map((item) => {
        return {
          ...
          buildingCodeName: this.getBuildingCodeName(item),
          totalParam: this.getTotalParam(item),
        };
      });
    },
  },
  methods: {
    getTotalParam(item) {
      try {
        return item.minimumNumberOfExitItems.length;
      } catch {}
      return 0;
      // return item.buildingCode.name;
    },
    getBuildingCodeName(item) {
      console.log("getBuildingCodeName", item);
      if (item.buildingCode == null) return "N/A";
      return item.buildingCode.name;
    },
    ...
    onEdit(item) {
      var self = this;
      self.$router.push({
        path: `/tenant/MinimumNumberOfExit/${item.id}`,
      });
    },
    ...
    addNew() {
      this.$router.push({ path: "/tenant/MinimumNumberOfExit" });
    },
    toast(header, message, color) {
      var self = this;
      self.infoList.push({
