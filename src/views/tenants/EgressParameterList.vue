  ...
  <div class="wrapper">
    ...
    <div>
      <CRow>
        <CCol sm="12">
          <CCard>
            <CCardHeader> <strong> EgressParameter </strong> List </CCardHeader>
            <CCardBody>
              <CDataTable
                :items="computedItems"
                :fields="fields"
...
export default {
  ...
  methods: {
    getTotalParam(item) {
      try {
        console.log(item);
        return item.paramExitStairWidthItems.length;
      } catch {}
      return 0;
    ...
    onEdit(item) {
      var self = this;
      self.$router.push({
        path: `/tenant/EgressParameter/${item.id}`,
      });
    },
    ...
    addNew() {
      this.$router.push({ path: "/tenant/EgressParameter" });
    },
    toast(header, message, color) {
      var self = this;
      self.infoList.push({
